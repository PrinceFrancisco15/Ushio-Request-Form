...
$sql = "SELECT * FROM request_forms WHERE request_id = ? AND 
        (manager_id = ? OR 
        (target_dept_manager_id = ? AND manager_status = 'approved' AND request_category = 'Access Derestriction'))";
$stmt = $conn->prepare($sql);
$stmt->bind_param("iii", $request_id, $manager_id, $manager_id);
$stmt->execute();
...
$sql = "SELECT l.*, requestor_name, u.role
...
<body>
    ...
    <div class="container mt-4">
        <div class="row">
                        ...
                        <div class="row mb-4">
                            <div class="col-md-6">
                                ...
                                <table class="table table-bordered">
                                    ...
                                    <th class="table-light">Target Department</th>
                <td>
                    <?php if($request['request_category'] == 'Access Derestriction' && !empty($request['target_department_id'])) {
                        // Fetch target department name
                        $target_dept_sql = "SELECT department_name FROM departments WHERE department_id = ?";
                        $target_stmt = $conn->prepare($target_dept_sql);
                        $target_stmt->bind_param("i", $request['target_department_id']);
                        $target_stmt->execute();
                        $target_result = $target_stmt->get_result();
                        $target_dept = $target_result->fetch_assoc();
                        echo $target_dept['department_name'];
                    } else {
                        echo 'N/A';
                    } ?>
                </td>
            ...
            <tr>
                <th class="table-light">Target Manager Approval</th>
                <td>
                    <?php if($request['request_category'] == 'Access Derestriction' && !empty($request['target_department_id'])) {
                        switch($request['target_manager_status']) {
                            case 'pending':
